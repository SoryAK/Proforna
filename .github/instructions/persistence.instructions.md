...
- **Content Requirements:**
  ...
  8. **Workflow Review Cadence:** Read `docs/chat-exports/analysis/workflow-reviews.json`, compute `daysSinceLastReview = floor((now - max(reviewedAt)) / 86400000)`, and report:
     ...
     If THIS session performed a workflow review (i.e. ran `scripts/analyze-chat-exports.mjs` and/or `npm run review:semantic` per the `session-self-review` recipe and shipped resulting rule edits and/or slice updates), you MUST also append an entry to `docs/chat-exports/analysis/workflow-reviews.json` BEFORE writing the handoff doc, with shape `{ reviewedAt: <ISO timestamp>, sessionId: <current session id>, summary: "<1-line>", ruleEditsShipped: ["<file>:<rule-slug>", ...], commits: ["<sha>", ...], sliceDriftAuditsRun?: ["<slug>:<shortSha>:<stillValid>/<stale>/<dropped>", ...] }`. The file is gitignored (lives under `docs/chat-exports/`) so the append is a local-only write — no commit required. The `sliceDriftAuditsRun` field is optional — omit if no Column 2 audits were run.
  9. **User Practice Adoptions:** This is the cohort-attribution forcing function for ADR-0036. Before writing the handoff doc, you MUST ask the user verbatim: *"Did this session introduce a new user practice, infrastructure change, or tool worth tracking in `workflow-change-log.json`?"* — and wait for an answer. Acceptable responses:
     - **Yes + brief description** → append one entry per item to `docs/chat-exports/analysis/workflow-change-log.json` under the appropriate stream (`userPracticeAdoptions` for human behavioural shifts, `infraToolingChanges` for tool/MCP/script changes, `agentRuleEdits` for shipped `.github/instructions/` rules). Each entry MUST include a stable `id` (kebab-case `slug-of-change`), `shippedAt` or `adoptedAt` (ISO timestamp), `summary` (one line), and `triggerSession` (current session id). Commit the change-log update as part of this session. Then render field #9 as: *"User Practice Adoptions: <id-1>, <id-2>, ... (appended to workflow-change-log.json)."*
     - **No / nothing new** → render field #9 as: *"User Practice Adoptions: probed — no new adoptions this session."* The word "none" is forbidden here.
     - **No answer / user skipped** → render: *"User Practice Adoptions: probe attempted; awaiting user response."* DO NOT silently default to "no" — the probe must be visible in the doc.
     Self-attribution rule: rules shipped by THIS session (i.e. commits made during this conversation that touched `.github/instructions/`) MUST also be appended to `agentRuleEdits` here, even though they will surface in commit 6 of the session-self-review recipe. Double-entry is prevented by the unique `id`.
- **Phase 4 Composite Gate:** The Handoff doc may only be written **after** BOTH of the following have run in the same response (or been explicitly waived by the user for that session):
  - **Memory Keeper** — proof = a visible `mcp_memory_*_open_nodes` tool call in this response.
  - **User Practice Adoptions probe** (field #9) — proof = the verbatim question above appears in this response AND a user reply has been received OR a `workflow-change-log.json` append has been made by the agent.
  Claim text alone is not proof for either. Without both, the Handoff doc must not be saved and the response must include the line: *"Phase 4 gate failed: <Memory Keeper | Practice Adoption probe | both> did not run — handoff deferred."* Audit data (2026-06-19): only 28.6% of sessions wrote to memory and 0% probed for practice adoptions. This composite gate exists to flip both ratios.
